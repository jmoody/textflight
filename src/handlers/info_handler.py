...
def handle_nav(c: Client, args: List[str]) -> None:
	...
	for planet in sys.planets:
		...
		if pname != None:
			...
			c.send(strings.INFO.PLANET_NAMED, index=index, name=pname, faction=pfact.name, planet_type=c.translate(planet.ptype.name))
		elif pfid != None:
			pfact = faction.get_faction(pfid)
			c.send(strings.INFO.PLANET_CLAIMED, index=index, name=pfact.name, planet_type=c.translate(planet.ptype.name))
		else:
...
def handle_status(c: Client, args: List[str]) -> None:
	...
	if report.mining_interval > 0:
		...
		c.send(strings.INFO.MINING_PROGRESS, progress=progress, interval="%.1f" % (report.mining_interval,))
