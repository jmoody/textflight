...
def get_system(sys: System) -> Tuple[int, str]:
	return get_planet(sys, -1)

def get_planet(sys: System, planet_id: int) -> Tuple[int, str]:
	ptup = conn.execute("SELECT * FROM faction_claims WHERE sys_id = ? AND planet = ?;", (sys.id_db, planet_id)).fetchone()
	if ptup != None:
		return (ptup["faction_id"], ptup["name"])
	return (None, None)

def set_system(sys: System, fid: int, name: str) -> None:
	return set_planet(sys, -1, fid, name)
